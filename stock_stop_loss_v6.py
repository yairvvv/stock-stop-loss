...
def calculate_momentum_metrics(df, period=14):
    ...
    return {
        ...
        "Breakout Score": round(min(breakout_score, 100), 1),
...
st.write("מערכת מעקב חכמה הכוללת חישובי תנודתיות (ATR), נפחי מסחר וציוני פריצה טכנולוגיים.")

if not st.session_state.stocks:
    st.info("אנא הוסף סימולי מניות דרך תפריט הצד כדי להתחיל בסריקה.")
else:
    results = []
    
    # תהליך הורדת הנתונים וחישוב המדדים
    with st.spinner("מוריד נתונים מ-Yahoo Finance ומנתח אינדיקטורים..."):
        for ticker in st.session_state.stocks:
            try:
                # הורדת נתוני היסטוריה של 3 חודשים לצורך חישוב הטווחים והאינדיקטורים
                ...
                st.error(f"שגיאה בניתוח הסימול {ticker}: {str(e)}")

    if results:
        # הפיכה ל-DataFrame לצורך תצוגה נוחה ומיון
        res_df = pd.DataFrame(results)
        res_df = res_df[['Ticker', 'Price', 'ATR', 'Breakout Score', 'Volume Ratio']]
        
        # מיון אוטומטי לפי המניות בעלות ציון הפריצה הגבוה ביותר
        res_df = res_df.sort_values(by="Breakout Score", ascending=False)
        
        # תצוגת ווידג'טים מהירים (Metrics) בחלק העליון
        st.subheader("📌 מבט מהיר על הרשימה")
        metric_cols = st.columns(min(len(results), 5))
        for idx, row in enumerate(res_df.itertuples()):
            col_idx = idx % min(len(results), 5)
            with metric_cols[col_idx]:
                st.metric(
                    label=row.Ticker, 
                    value=f"${row.Price}", 
                    delta=f"Score: {row.Breakout Score}",
                    delta_color="normal" if row.Breakout_Score > 70 else "off"
                )
        
        st.markdown("---")
        
        # טבלת הנתונים המרכזית עם צביעה מותנית
        st.subheader("📊 טבלת סריקה והשוואה")
        st.write("המניות ממוינות לפי עוצמת המומנטום והקרבה לפריצת שיא מקומי:")
        
        # צביעת עמודת הציון להדגשת הזדמנויות
        styled_df = res_df.style.background_gradient(
            subset=['Breakout Score'], 
            cmap='RdYlGn', 
            vmin=0, 
            vmax=100
        )
        ...
        # שורת הסבר קצרה בתחתית
        st.caption("💡 *טיפ החזקה:* ציון פריצה (Breakout Score) קרוב ל-100 מעיד על מחיר הנושק לשיא של 20 ימים. יחס נפח מסחר (Volume Ratio) מעל 1.0 מעיד על עניין גובר של קונים.")